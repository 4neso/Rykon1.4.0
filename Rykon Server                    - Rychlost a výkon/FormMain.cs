...
namespace RykonServer
{
 ...
 public  partial class   FormMain : Form
 {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        else if (cp.LocalPath.StartsWith("/Control"))
                        {
                            ...
                            if (!this.Servconf.EnableControler) // disabled
                            {
                            ...
                            {
                                validauthtok =cp.UrlOriginalString.Contains(this.CSRF);
                                hasthepass =cp.UrlOriginalString.Contains(Servconf.ControlPassword);

                                if (this.Servconf.SecureControl)
                                    AllowedTocontrol =(validauthtok && hasthepass);
                                else
                                ...
                                if (!AllowedTocontrol)
                                {
                                    ...
                                    cp.Output_document = (!validauthtok && cp.UrlOriginalString.Contains("CSRF"))?WebDesigner.invalidAuthTok:WebDesigner.ControlNotAllowedIndex(CSRF);//ControlLoginPage;
                                ...
                                if (pcs.Length > 0)
                                {
                                    ...
                                    if (main.StartsWith(this.CSRF))
                                        main = main.Substring(this.CSRF.Length);

                                    RemoteCommandExecuter r = new RemoteCommandExecuter(main);
                                    r.HandlePointer = this.Handle;
                                    r.proceeed();
        ...
        private void GenerateControlIndex()
        {
            ...
            string d = WebDesigner.ControlCommandListindex(this._RootDirectory, this._Prefixs_[(cb_Prefixs.SelectedIndex)].Item2, this._Port.ToString(), this.CSRF,this.Servconf.ControlPassword);
            bool b = AppHelper.writeToFile(mp, d);
